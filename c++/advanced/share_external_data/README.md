# 运行 ShareExternalData 样例

ShareExternalData 样例展示了在 CPU 和 GPU 下使用 `ShareExternalData` 接口进行推理的过程。运行步骤如下：
...
文件`resnet50_share_data.cc` 为预测的样例程序（程序中的输入为固定值，如果您有opencv或其他方式进行数据读取的需求，需要对程序进行一定的修改）。    
