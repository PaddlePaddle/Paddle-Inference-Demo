# 运行 Managed Memory 预测样例

以 ResNet50 为例展示使用 Managed Memory 进行预测的过程。运行步骤如下：
...
- 文件`managed_memory.cc` 为预测的样例程序（程序中的输入为固定值，如果您有opencv或其他方式进行数据读取的需求，需要对程序进行一定的修改）。    
