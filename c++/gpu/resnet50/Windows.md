...
ResNet50 样例展示了单输入模型在 CPU 下使用 GPU 的推理过程。本文档基于`Visual Studio 2019`编写，最好使用相同版本运行样例。运行步骤如下：
...
### 1. 编译准备
环境需要支持Ninja构建，需要先将`Paddle-Inference-Demo/c++/lib`下的`CMakeLists.txt`拷贝至本目录
...
cmake .. -G "Ninja" -DCMAKE_BUILD_TYPE=Release -DWITH_GPU=ON -DWITH_MKL=ON -DDEMO_NAME=resnet50_test -DWITH_SHARED_PHI==ON -DWITH_STATIC_LIB=OFF -DPADDLE_LIB=path/to/paddle_inference

# 执行编译
ninja
```
执行完之后在`build`目录下出现`resnet50_test.exe`就是编译出来的可执行文件。
