...
DEFINE_string(hs, "608", "input heights, separeted by ':'");
DEFINE_string(ws, "608", "input widths, separeted by ':'");
DEFINE_string(no_seen_hs, "416", "no seen input heights, separeted by ':'");
DEFINE_string(no_seen_ws, "416", "no seen input widths, separeted by ':'");
  ...
  if (FLAGS_use_gpu) {
    ...
    if (FLAGS_use_trt) {
      ...
      if (FLAGS_tuned_dynamic_shape) {
	      config->Exp_DisableTensorRtOPs({"roi_align"});
...
int main(int argc, char **argv) {
  ...
  for (size_t b = 1; b <= FLAGS_max_batch_size; b++) {
    for (size_t i = 0; i < hs.size(); ++i) {
      ...
      input_infos[in_names[1]] = std::make_pair(shape, in_data);

      std::vector<int> im_shape{b, 2};
      std::vector<float> im_shape_data(b * 2);
      for (int i = 0; i < b; ++i) {
          im_shape_data[i*2+0] = h;
          im_shape_data[i*2+1] = w;
      }
      input_infos[in_names[0]] = std::make_pair(im_shape, im_shape_data);

      std::vector<int> scale_factor_shape{b, 2};
      std::vector<float> scale_factor_data(b * 2);
      for (int i = 0; i < b; ++i) {
          scale_factor_data[i*2] = h / 416.f;
          scale_factor_data[i*2+1] = w / 416.f;
      }
      input_infos[in_names[2]] = std::make_pair(scale_factor_shape, scale_factor_data);
