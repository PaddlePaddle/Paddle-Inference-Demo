...
安装 Paddle Inference 主要包括 **下载安装预测库** 和 **源码编译** 两种方式。  

 **下载安装预测库** 是最简单便捷的安装方式，Paddle Inference 提供了多种环境组合下的预编译库，如 cuda/cudnn 的多个版本组合、是否支持 TensorRT 、可选的 CPU 矩阵计算加速库等。详细内容可参考以下文档：

- 下载/安装预测库：

    - `Python安装 <python_install.html>`_ : 使用python wheel安装包直接安装  
    - `C++预测库 <cpp_install.html>`_ : 下载并使用C++预编译库  
    - `C预测库 <c_install.html>`_ : 下载并使用C预编译库  
