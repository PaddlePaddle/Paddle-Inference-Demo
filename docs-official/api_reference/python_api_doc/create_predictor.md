...
# save_optimized_model 方法

`save_optimized_model` 接口可保存经过选定 backend 硬件 analysis ir pass 优化的模型结构和模型参数，API 定义如下：
...
# 返回：NONE
paddle.inference.save_optimized_model(
    model_file,
    params_file,
    output_model_file,
    output_params_file,
...
```python
from paddle.inference import PlaceType
from paddle.inference import save_optimized_model
import argparse
...
if __name__ == "__main__":
    ...
    save_optimized_model(
        args.src_model,     # 原始模型文件路径
        args.src_params,    # 原始权重文件路径
        args.dst_model,     # 优化后的模型文件保存路径
        args.dst_params,    # 优化后的权重文件保存路径
        PlaceType.XPU,      # 硬件后端，如 PlaceType.XPU
        black_list          # 黑名单列表，哪些 pass 不需要执行
    )
```
